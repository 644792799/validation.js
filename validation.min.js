...
{return obj.value!==undefined?obj.value:false;}},create_error_message:function(target,message){this.errors[target.id]=message;if(!message||!this.options.error_display){return;}var parent=target.parentNode,text=document.createTextNode(message),msg=document.createElement(this.options.error_node);msg.setAttribute('id','error__'+target.id);msg.className=this.options.error_class;msg.appendChild(text);if(this.options.error_placement=='before'){var before=parent.firstChild;parent.insertBefore(msg,before);}else
